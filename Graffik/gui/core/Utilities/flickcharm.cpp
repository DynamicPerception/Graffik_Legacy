...
/* Changes by C.A. Church:

    * Removed QWebView-related code.
    * Added signal emitted when scrolling stops
...
static QPoint deaccelerate(const QPoint &speed, int a = 1, int max = FLICK_MAX_SPD)
{
    ...
    case FlickData::Pressed:
        ...
        if (mouseEvent->type() == QEvent::MouseMove) {
            ...
            if (!d->ticker.isActive())
                d->ticker.start(FLICK_TIMER_MS, this);
        }
        break;
